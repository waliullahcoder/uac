    ...
    <tr id="sales_{{ $item->id }}">
        ...
        <td style="padding: 4px 0.25rem;">
            <input type="number" class="form-control input-sm text-end previous_paid" step="any"
                ...
                value="{{ $item->paid }}" placeholder="0.00" readonly>
        </td>
        @if (@$type == 'Adjust')
            @php
                $checked = true;
            ...
            <td style="padding: 4px 0.25rem;">
                <input type="number" class="form-control input-sm text-end amount" step="any"
                    ...
                    max="{{ $item->net_amount - $item->return_amount - $item->paid }}" min="0"
                    value="{{ $paid }}" placeholder="0.00">
            </td>
            <td style="padding: 4px 0.25rem;">
            ...
            <td style="padding: 4px 0.25rem;" class="text-center">
                <div class="custom-control custom-checkbox mx-auto ps-3">
                    <input type="checkbox" class="custom-control-input sales_id" id="sales_id_{{ $item->id }}"
                        name="sales_id[]" data-payable="{{ $item->net_amount - $item->return_amount - $item->paid }}"
            ...
            <td style="padding: 4px 0.25rem;" class="text-center">
                <div class="custom-control custom-checkbox mx-auto ps-3">
                    <input type="checkbox" class="custom-control-input sales_id" name="sales_id[]"
                        data-payable="{{ $item->net_amount - $item->return_amount - $item->paid }}"
                        value="{{ $item->id }}" id="{{ $item->id }}">
                    <label for="{{ $item->id }}" class="custom-control-label"></label>
                </div>
            </td>
